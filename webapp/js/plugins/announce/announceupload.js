...
function addAsynchronousUploadField(fieldId) {
	...
	if ( swfobject.hasFlashPlayerVersion( "9.0.24" ) )
	{
	    $('#' + fieldId).uploadify({
			...
			*/
	        'uploader' : 'js/plugins/announce/uploadify/swf/uploadify.swf',
	        'script' : baseUrl + '/jsp/site/upload',
	        'cancelImg' : 'js/plugins/announce/uploadify/cancel.png',
			'auto' : true,
			...
			'scriptData' : {'jsessionid' : document.cookie.match(/JSESSIONID=([^;]+)/)[1], 'page': 'announce', 'fieldname':fieldId},
		...
		$( '#_announce_upload_submit_' + fieldId ).hide();
...
function formDisplayUploadedFiles( jsonData )
{
	...
	if ( fieldName != null )
	{
		if ( jsonData.fileCount == 0 ){
			...
			var strContent = "";
			var checkboxPrefix = '_announce_upload_checkbox_' + fieldName;
...
$('button').each(function() {
	$(this).click(function(event) {
			...
			else if ( this.name.match("_announce_upload_delete_"  ) )
			{
				event.preventDefault(); 
...
function getMaxFiles( fieldId ) {
	return getInputValue( '#_announce_upload_maxFiles_' + fieldId );
}
...
function getMaxLengthValue( fieldId ) {
	return getInputValue( '#_announce_upload_maxLength_' + fieldId );
}
...
function removeFile( action ) {
	var fieldName = action.match("_announce_upload_delete_attribute_(.*)")[1];
	
	// build indexes to remove
	var indexes = new Array();
	
	var indexesCount = 0;
	var checkboxPrefix = '_announce_upload_checkbox_attribute_' + fieldName;
	...
	$.getJSON(baseUrl + 'jsp/site/plugins/announce/DoRemoveFile.jsp', jsonData,
		function(json) {
...
function keepAlive(  ) {
	if ( uploading > 0 )
	{
		$.getJSON(baseUrl + 'jsp/site/plugins/announce/KeepAlive.jsp');
	}
